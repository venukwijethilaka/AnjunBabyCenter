    ...
    const handlePromoteDemote = async (user: User) => {
      ...
      const handleBanClick = (user: User) => {
        if (user.isActive) {
            setBanData({ 
                userId: user.id, 
                userName: user.name ?? 'User', // Fallback for state
                reason: '', 
                duration: 7 
            });
            setIsBanModalOpen(true);
        } else {
            // Fallback for the browser confirm dialog
                                        ...
                                        <tr key={user.id} className="hover:bg-blue-50/30 transition-colors">
                                            <td className="p-5">
                                                <div className="flex items-center gap-4">
                                                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-white shadow-md shadow-gray-200 ${user.role?.includes('ADMIN') ? 'bg-gradient-to-br from-purple-500 to-indigo-600' : 'bg-gradient-to-br from-blue-500 to-cyan-500'}`}> {user.name?.[0]?.toUpperCase() ?? "U"}</div>
