...
import { X, Upload, Loader2, Star, Trash2, Edit } from 'lucide-react'
...
interface EditProductModalProps {
  ...
  product?: Product; categories?: Category[]; isLoading?: boolean;
}

const EditProductModal = ({ isOpen, onClose, onSubmit, product, categories = [], isLoading = false }: EditProductModalProps) => {
  const [formData, setFormData] = useState({ name: '', description: '', price: '', quantity: '', images: [] as ProductImage[], color: '', size: '', categoryId: '', availability: true, discountPercentage: '' })
    ...
    return (
      <div className="fixed inset-0 bg-red-100 flex items-center justify-center z-50 p-4">
        <div className="text-red-700 text-center bg-white p-8 rounded-3xl shadow-2xl">
          ...
          <p>The ImageKit environment variables are missing.</p>
        </div>
      </div>
    );
  }
  ...
  useEffect(() => {
    if (product && isOpen) {
      setFormData({ name: product.name, description: product.description, price: String(product.price), quantity: String(product.quantity), images: product.images || [], color: product.color || '', size: product.size || '', categoryId: String(product.categoryId), availability: product.availability, discountPercentage: product.discountPercentage ? String(product.discountPercentage) : '' })
    }
  }, [product, isOpen])
  ...
  const onUploadSuccess = (res: any) => { setUploadingCount(prev => Math.max(0, prev - 1)); setFormData(prev => { const newImg: ProductImage = { url: res.url, isMain: prev.images.length === 0 }; return { ...prev, images: [...prev.images, newImg] } }) }
  const onUploadError = () => { setUploadingCount(prev => Math.max(0, prev - 1)); alert("Upload failed.") }
  const removeImage = (index: number) => { setFormData(prev => { const newImgs = prev.images.filter((_, i) => i !== index); if (prev.images[index]?.isMain && newImgs.length > 0) newImgs[0].isMain = true; return { ...prev, images: newImgs }; }); }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ name: formData.name, description: formData.description, price: parseFloat(formData.price), quantity: parseInt(formData.quantity), color: formData.color || null, size: formData.size || null, categoryId: parseInt(formData.categoryId), availability: formData.availability, discountPercentage: formData.discountPercentage ? parseFloat(formData.discountPercentage) : null, images: formData.images.map(img => ({ url: img.url, isMain: img.isMain, altText: formData.name })) });
  }
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 font-medium line-clamp-1">{product?.name}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-8 custom-scrollbar flex-1">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="col-span-1 space-y-4">
              <label className="block text-xs font-black text-slate-400 uppercase tracking-widest">Gallery</label>
              <div className="grid grid-cols-2 gap-3">
                {formData.images.map((img, index) => (
                  <div key={index} className={`group relative aspect-square rounded-2xl border-2 overflow-hidden bg-slate-50 ${img.isMain ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200'}`}>
                    <img src={img.url} alt="Product" className="w-full h-full object-cover" />
                    {img.isMain && <div className="absolute top-2 left-2 bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded-full font-black z-10">Main</div>}
                ...
                <IKContext publicKey={process.env.NEXT_PUBLIC_IMAGEKIT_PUBLIC_KEY} urlEndpoint={process.env.NEXT_PUBLIC_IMAGEKIT_URL} authenticator={authenticator}>
                  <label className="relative aspect-square border-2 border-dashed border-blue-200 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all group">
                    {uploadingCount > 0 ? <Loader2 className="animate-spin text-blue-500" size={24} /> : <><Upload className="text-blue-300 group-hover:text-blue-500 mb-1 transition-colors" size={24} /><span className="text-[10px] text-slate-400 font-bold">Add More</span></>}
            ...
            <div className="col-span-2 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-200 outline-none font-semibold transition-all" required />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Price (Rs.)</label>
                  <input type="number" name="price" value={formData.price} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-200 outline-none font-semibold transition-all" step="0.01" required />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Stock Qty</label>
                  <input type="number" name="quantity" value={formData.quantity} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-200 outline-none font-semibold transition-all" required />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Color</label>
                  <input type="text" name="color" value={formData.color} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-200 outline-none font-semibold transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Size</label>
                  <input type="text" name="size" value={formData.size} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-200 outline-none font-semibold transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Discount %</label>
                  <input type="number" name="discountPercentage" value={formData.discountPercentage} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-200 outline-none font-semibold transition-all" />
                </div>
                <div className="col-span-2">
              ...
              {/* Algorithm info banner — replaces boolean checkboxes */}
