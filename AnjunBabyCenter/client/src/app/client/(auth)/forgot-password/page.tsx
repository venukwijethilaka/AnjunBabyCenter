...
export default function ForgotPasswordPage() {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await sendCodeApi({ email }).unwrap();
      // Save the hash and expiry
      setVerificationData({ hash: response.hash, expiry: response.expiry });
      
      setStep(2); 
      setCountdown(60);
      showNotification("Code sent! Check your email.", 'success');
    } catch (err: any) { showNotification(err?.data?.message || "Failed to send code.", 'error'); }
  };
  ...
  const handleReset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (otp.length !== 6) { showNotification("Please enter the complete 6-digit code.", 'error'); return; }

    try {
      // Pass the hash and expiry back to reset password
    ...
    } catch (err: any) { showNotification(err?.data?.message || "Failed to reset password.", 'error'); }
  };
  
  return (
    <div className="min-h-[100dvh] bg-gray-50/40 flex items-center justify-center p-4 relative">
      
      {/* Notification Toast */}
      {notification && (
        <div className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-[9999] px-6 py-4 rounded-2xl shadow-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-300 ${
            notification.type === 'success' 
              ? 'bg-theme-toggle-bg text-theme-primary border border-theme-border' 
      ...
      <div className="w-full max-w-md bg-white p-8 sm:p-10 rounded-[32px] shadow-sm border border-gray-100">
        ...
        {step === 1 ? (
          ...
          <form onSubmit={handleSendCode} className="space-y-6">
            <Input 
              ...
              required 
            />
            <Button type="submit" isLoading={isSending} className="w-full">Send Reset Code</Button>
            <button 
              type="button"
              onClick={() => router.push("/client/sign-in")}
              className="w-full text-center text-sm text-gray-500 hover:text-theme-primary transition-colors mt-4 font-bold"
            >
              Back to Login
            </button>
          </form>
          ...
          <form onSubmit={handleReset} className="space-y-6">
             <div className="p-4 bg-theme-toggle-bg text-theme-primary text-sm rounded-2xl text-center border border-theme-border font-medium">
            ...
            <div className="space-y-5">
                <Button type="submit" isLoading={isResetting} className="w-full">Change Password</Button>
            ...
            <button 
              type="button" 
              onClick={() => setStep(1)} 
              className="w-full text-center text-sm text-gray-500 hover:text-theme-primary transition-colors font-bold"
            >
              Wrong Email? Go Back
            </button>
          </form>
